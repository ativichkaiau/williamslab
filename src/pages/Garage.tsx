  ...
  return (
    <>
      <div className="page-head">
        <Rule />
        <Kicker>R&amp;D GARAGE · PROJECT HOME</Kicker>
        <h1>{state.project.name}</h1>
        <p>{state.project.domain} · {state.project.code}</p>
      </div>

      <div className="grid g4" style={{ marginBottom: 16 }}>
        <StatCard value={`${Math.round(stability * 100)}%`} label="Chassis stability" sub={`${open.length} open flags`} />
        <StatCard value={state.hypotheses.length} label="Hypotheses" sub="on the graph" />
        <StatCard value={state.assays.length} label="Assays planned" sub={`${state.assays.filter((a) => a.status === 'running' || a.status === 'piloting').length} in the pit lane`} />
      ...
      <div className="grid g2">
        <div className="card lg">
          ...
          <div className="wrap-gap">
            <Link className="btn primary sm" to="/pit-wall">Open the Pit Wall →</Link>
        ...
        <div className="card lg">
          ...
          {highs.length === 0 && <p className="empty">No high-severity flags — chassis is planted.</p>}
          ...
          <div className="divider" />
          <Link className="btn ghost sm" to="/suspension">Go to Active Suspension →</Link>
        </div>
      </div>

      <div className="card lg" style={{ marginTop: 16 }}>
        <div className="card-h"><span className="sq" style={{ background: 'var(--yellow)' }} />THE LAP · WHERE THIS PROJECT SITS</div>
        <div className="wrap-gap">
